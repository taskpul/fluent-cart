...
class LicenseGenerationHandler
{
    ...
    private function addNewLicense($licenseData, $orderModel, $subscription = null)
    {
        ...
        foreach ($orderModel->order_items as $item) {
            if ($item->payment_type == 'bundle' && $item->object_id == $licenseData['variation_id']) {
                $isBundleOrderItem = true;
                break;
            }
        }
        ...
        } else if ($isBundleOrderItem && !$subscription) { 
            $data['expiration_date'] = null; // overriding item's expiration date for bundle order, if the bundle order is not a subscription
        }
