...
class MollieIPN
{
    ...
    public function handlePaymentPaid($data)
    {
        ...
        $subscriptionData = [];
        if ($mandateId && $transaction->subscription_id) {
            $subscriptionModel = Subscription::query()->where('id', $transaction->subscription_id)->first();
            if ($subscriptionModel && empty($subscriptionModel->vendor_subscription_id)) {
                $billingInfo = [
                ...
                $subscriptionData = (new Confirmations())->handleSubscriptionCreation($transaction, [
                    'mandate_id' => $mandateId,
    ...
    public function handlePaymentAuthorized($data)
    {
        ...
        return (new Confirmations())->authorizePaymentByCharge($transaction, [
            'vendor_charge_id' => $paymentId,
            'charge' => $payment,
            'subscription_data' => $subscriptionData
        ]);
