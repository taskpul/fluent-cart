...
use FluentCart\App\Models\Product as FctProduct;
...
class Price
{
    public static function getOrCreatePaddleSinglePrice($data = [])
    {
        $item = ProductItemService::getItem([
           ...
           'product_id' => Arr::get($data, 'fct_product_id'),
        ]);
        ...
        if ($product && $type === 'standard') {
            ...
            $paddlePriceId = null;
            if ($product && $product instanceof FctProduct) {
                $paddlePriceId = $product->getProductMeta($priceId);
            }
        ...
        if ($product && $type === 'standard' && $product instanceof FctProduct) {
    ...
    public static function getOrCreatePaddleRecurringPrice($data = [])
    {
        ...
        if ($type === 'standard' && $product && $product instanceof Product) {
            $product->updateProductMeta($priceId, Arr::get($paddlePrice, 'data.id'));
        }
        return $paddlePrice;
    }
