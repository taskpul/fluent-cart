...
class FluentLicensing
{
    ...
    public function activate($licenseKey = '')
    {
        ...
        // Add success notice to response
        $saveData['notice'] = [
            'id'      => 'license_activation_success',
            'type'    => 'license_activation_success',
            'html'    => '<div>License activated successfully.</div>',
            'timeout' => 1000,
        ];
    ...
    public function deactivate()
    {
        ...
        if (is_wp_error($deactivated)) {
            return $deactivated; // Return the error response if there is an error.
        }

        delete_option($this->settingsKey); // Remove the license data from the database.

        // Fetch license notice back
        $licenseNotice = $this->getLicenseNotice();

        // Add deactivate notice to response
        $deactivated['notice'] = [
            'id'   => 'deactivate_license',
            'type' => $licenseNotice['type'] ?? 'global',
            'html' => wp_kses_post($licenseNotice['message']),
        ];
    ...
    public function getLicenseNotice()
    {
        ...
        if ($status != 'valid') {
            return [
                'id'            => 'activate_license',
                'message' => sprintf(
                    ('The %1$s license needs to be activated. %2$sActivate Now%3$s'),
